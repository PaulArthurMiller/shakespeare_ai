...
class Assembler:
    def __init__(
        ...
        self.logger = logger or CustomLogger("Assembler")
    ...
    def assemble_line(self, modern_line: str, prompt_data: Dict[str, List[Dict[str, Any]]], max_retries: int = 1) -> Optional[Dict[str, Any]]:
        ...
        # Log the structure of prompt_data
        ...
        while retries <= max_retries:
            ...
            # ADD THIS: Log the state of working_prompt_data before mini-validation
            self.logger.debug("State of working_prompt_data before mini-validation:")
            for key, items in working_prompt_data.items():
                if key == "metadata":
                    continue  # Skip metadata
                self.logger.debug(f"  '{key}' has {len(items)} items")
                if items:
                    sample_temp_ids = [item.get("temp_id", "NO_TEMP_ID") for item in items[:3]]
                    self.logger.debug(f"  Sample temp_ids: {sample_temp_ids}")
    ...
    def _mini_validate(self, assembled_line: str, quote_data: Dict[str, List[Dict[str, Any]]]) -> Union[Dict[str, Any], bool]:
        ...
        # ADD THIS: Log detailed structure of quote_data
        self.logger.debug(f"quote_data keys: {list(quote_data.keys())}")
        for key, quotes in quote_data.items():
            if key == "metadata":
                continue  # Skip metadata
            self.logger.debug(f"quote_data['{key}'] has {len(quotes)} items")
            # Check structure of the first quote in each category
            if quotes:
                first_quote = quotes[0]
                self.logger.debug(f"  First quote in '{key}' has keys: {list(first_quote.keys())}")
                self.logger.debug(f"  First quote has temp_id: {first_quote.get('temp_id', 'MISSING')}")
                self.logger.debug(f"  First quote text: '{first_quote.get('text', 'MISSING')[:30]}...'")
        ...
        for form, quotes in quote_data.items():
            ...
            for quote in quotes:
                # ADD THIS: Check if quote has required keys
                if not isinstance(quote, dict):
                    self.logger.warning(f"Quote is not a dictionary: {type(quote)}")
                    continue
                    
                if "temp_id" not in quote:
                    self.logger.warning(f"Quote missing temp_id: {quote}")
                    continue
                    
                if "text" not in quote:
                    self.logger.warning(f"Quote missing text: {quote}")
                    continue
        ...
        # ADD THIS: Log all_quotes details
        self.logger.debug(f"Collected {len(all_quotes)} normalized quotes for validation")
        for i, q in enumerate(all_quotes[:3]):  # Log first 3 for brevity
            self.logger.debug(f"  Quote {i+1}: temp_id={q['temp_id']}, normalized='{q['normalized'][:30]}...'")
        ...
        for position in range(3):
            ...
            found_match = False
            self.logger.debug(f"Position {position+1}: Trying to match remaining text: '{remaining_text[:50]}...'")
            self.logger.debug(f"Position {position+1}: Have {len(available_quotes)} quotes to try")
            ...
            for i, quote in enumerate(available_quotes):
                ...
                if remaining_text.startswith(quote["normalized"]):
                    ...
                    self.logger.debug(f"Match at position {position+1}: {quote['temp_id']} - '{quote['original'][:30]}...'")
            ...
            if not found_match and remaining_text:
                ...
                self.logger.warning(f"No matching quote found at the beginning of remaining text: '{remaining_text[:50]}...'")
                return False
